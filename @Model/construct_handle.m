...
% meanr       Mean of the PSD     5           7
% pdist       Polydispersity      6           8

% All parameters fixed:
% @(prm,q)scattered_intensity(q,a,nc,tau,vskin,fuzz,psd_m,psd_w)
% All paramters bound:
%                                  1   2      3      4      5      6      7      8   
% @(prm,q) obj.scattered_intensity(q,prm(4),obj.nc,prm(1),prm(2),prm(3),prm(5),prm(6));
...
str = ['@(prm,q) obj.scattered_intensity(q,' call{4} ',obj.nc,' call{1} ',' call{2} ',' call{3} ',' call{5} ',' call{6} ')'];
