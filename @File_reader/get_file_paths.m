function p = get_file_paths(obj,fs)
%GET_FILE_PATHS Opens a standard open file dialog and returns the file paths
%of the selected files
%   p = getFilePaths(fs), where fs is a cell array of strings containing
%   file extensions that can be imported, returns cell array of strings p
%   containing the full paths to the selected files.
%
%   Throws:
%       FitIt:UIException:Open dialog cancelled.

[filenames, pathnames, ~] = uigetfile(fs,'Select files for import','MultiSelect','off',obj.last_load_path);
